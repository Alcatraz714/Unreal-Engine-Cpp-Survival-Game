...
AEnemy::AEnemy()
{
	...
	AgroSphere->SetCollisionResponseToChannel(ECollisionChannel::ECC_WorldDynamic, ECollisionResponse::ECR_Ignore);//agro sphere does not trigger explosives
	...
	// attach to the socket for claws and weapons - old method removed from constructor
	//CombatCollision->AttachToComponent(GetMesh(), FAttachmentTransformRules::SnapToTargetIncludingScale, FName("EnemySocket"));
	//rule to scale the mesh with parent of socket
	...
	if (OtherActor)
	{
		AMainCharacter* Main = Cast<AMainCharacter>(OtherActor);
		{
			if (Main)
			{
				...
				Main->SetHasCombatTaregt(false);//set fasle for enemy out of range
				
				Main->UpdateCombatTarget();//remove health bar after moving out
...
void AEnemy::CombatOnOverlapBegin(UPrimitiveComponent* OverlappedComponent, AActor* OtherActor, UPrimitiveComponent* OtherComp, int32 OtherBodyIndex, bool bFromSweep, const FHitResult& SweepResult)
{
	if (OtherActor)
	{
		...
		if (Main)
		{
			...
			if (DamageTypeClass)
			{
				UGameplayStatics::ApplyDamage(Main, Damage, AIController, this, DamageTypeClass);//params on main and this for the enemy applying damage
			}
		}
	}
}
...
void AEnemy::Die(AActor* Causer)
{
	...
	if (AnimInstance)
	{
		...
		AnimInstance->Montage_JumpToSection(FName("Death"), CombatMontage);//jump to attack section
	}
